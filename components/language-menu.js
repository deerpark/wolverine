  ...
  const localeItems = LOCALES.map((locale, index) => {
    ...
    return (
      <DropdownItem
        ...
        onClick={() => handleClickToLoad(locale.value)}
        href={`/${locale.value}/ranking/${_query.ratings}/${_query.date}`}
        key={index}
      >
        <i className={`flag ${locale.value}`} />
        {locale.label}
      </DropdownItem>
    );
  });
...
export default ({ query, handleClickToLoad }) => (
  <>
    <DropdownMenu
      trigger={
        <>
          <i className={`link flag`} /> <i className="fas fa-caret-down" />
        </>
      }
      triggerType="default"
      shouldFlip={true}
